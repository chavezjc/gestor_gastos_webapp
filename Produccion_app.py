...
load_dotenv() # <--- DEBE ESTAR AQUÍ, MUY AL PRINCIPIO, DESPUÉS DE LAS IMPORTACIONES ARRIBA
import logging # <--- Y esta también, si mantuviste los logs de depuración
...
# ==================================================================
app = Flask(__name__)
# Cargar SECRET_KEY desde variable de entorno (sin valor por defecto)
app.secret_key = os.environ.get('FLASK_SECRET_KEY') # <--- CAMBIO AQUÍ: ELIMINAR EL VALOR POR DEFECTO
# Configuración Google OAuth (sin valores por defecto)
app.config["GOOGLE_OAUTH_CLIENT_ID"] = os.environ.get("GOOGLE_OAUTH_CLIENT_ID") # <--- CAMBIO AQUÍ
app.config["GOOGLE_OAUTH_CLIENT_SECRET"] = os.environ.get("GOOGLE_OAUTH_CLIENT_SECRET") # <--- CAMBIO AQUÍ
# Cargar SQLALCHEMY_DATABASE_URI desde variable de entorno (sin valor por defecto)
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL') # <--- CAMBIO AQUÍ
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
# Inicialización de SQLAlchemy y Flask-Login
db = SQLAlchemy(app) # <--- ¡MUEVE ESTA LÍNEA AQUÍ!
login_manager = LoginManager()
login_manager.init_app(app)
login_manager.login_view = 'hello_world'
migrate = Migrate(app, db) # <--- ¡ASEGÚRATE DE QUE ESTA LÍNEA ESTÉ AQUÍ Y ESTÉ CORRECTA!
import logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
# --- BLOQUE DE DEPURACIÓN DE VARIABLES ---
logging.info(f"DEBUG APP: SECRET_KEY (from os.environ): {os.environ.get('FLASK_SECRET_KEY')}")
logging.info(f"DEBUG APP: DATABASE_URL (from os.environ): {os.environ.get('DATABASE_URL')}")
logging.info(f"DEBUG APP: SQLALCHEMY_DATABASE_URI (app.config): {app.config['SQLALCHEMY_DATABASE_URI']}")
# --- FIN BLOQUE DE DEPURACIÓN DE VARIABLES ---
# Inicialización de Blueprint de Flask-Dance
google_bp = make_google_blueprint(
    scope=["openid", "https://www.googleapis.com/auth/userinfo.email", "https://www.googleapis.com/auth/userinfo.profile"],
    # CAMBIO: Redirecciona al login_view de Flask-Login para manejar el flujo si no hay Google Auth exitoso
    redirect_to="hello_world" # Asegura que si falla o no hay auth, vuelve a la ruta principal
)
